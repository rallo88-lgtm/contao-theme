...
class RctTabsJsonStorageMigration extends AbstractMigration
{
    private const FIELDS   = ['rct_tabs_data', 'rct_tabs_color', 'rct_tabs_style'];
    private const CE_TYPES = ['rct_tabs'];

    public function __construct(private readonly Connection $db) {}

    public function getName(): string
    {
        return 'RCT Bundle – rct_tabs fields to jsonData column';
    }
    ...
    public function shouldRun(): bool
    {
        ...
        $unmigrated = (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM tl_content
             ...
             AND (jsonData IS NULL OR JSON_EXTRACT(jsonData, '$.rct_tabs_color') IS NULL)",
            self::CE_TYPES
        );
        return $unmigrated > 0;
    }
    ...
    public function run(): MigrationResult
    {
        ...
        return $this->createResult(
            true,
            sprintf('rct_tabs → jsonData: %d Element(e) migriert.', $touched)
        );
    }
}
