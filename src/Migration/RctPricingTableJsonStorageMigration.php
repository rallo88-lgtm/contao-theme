...
class RctPricingTableJsonStorageMigration extends AbstractMigration
{
    private const FIELDS   = ['rct_pricing_data', 'rct_pricing_style'];
    private const CE_TYPES = ['rct_pricing_table'];

    public function __construct(private readonly Connection $db) {}

    public function getName(): string
    {
        return 'RCT Bundle – rct_pricing_table fields to jsonData column';
    }
    ...
    public function shouldRun(): bool
    {
        ...
        $unmigrated = (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM tl_content
             ...
             AND (jsonData IS NULL OR JSON_EXTRACT(jsonData, '$.rct_pricing_style') IS NULL)",
            self::CE_TYPES
        );
        return $unmigrated > 0;
    }
    ...
    public function run(): MigrationResult
    {
        ...
        return $this->createResult(
            true,
            sprintf('rct_pricing_table → jsonData: %d Element(e) migriert.', $touched)
        );
    }
}
