...
class RctMapJsonStorageMigration extends AbstractMigration
{
    private const FIELDS   = ['rct_map_address', 'rct_map_marker', 'rct_map_zoom', 'rct_map_height'];
    private const CE_TYPES = ['rct_map'];

    public function __construct(private readonly Connection $db) {}

    public function getName(): string
    {
        return 'RCT Bundle – rct_map fields to jsonData column';
    }
    ...
    public function shouldRun(): bool
    {
        ...
        $unmigrated = (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM tl_content
             ...
             AND (jsonData IS NULL OR JSON_EXTRACT(jsonData, '$.rct_map_zoom') IS NULL)",
            self::CE_TYPES
        );
        return $unmigrated > 0;
    }
    ...
    public function run(): MigrationResult
    {
        ...
        return $this->createResult(
            true,
            sprintf('rct_map → jsonData: %d Element(e) migriert.', $touched)
        );
    }
}
