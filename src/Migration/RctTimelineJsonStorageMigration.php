...
class RctTimelineJsonStorageMigration extends AbstractMigration
{
    private const FIELDS = [
        'rct_timeline_data',
        'rct_timeline_color',
        'rct_timeline_variant',
        'rct_timeline_show_line',
        'rct_timeline_style',
    ];
    private const CE_TYPES = ['rct_timeline'];

    public function __construct(private readonly Connection $db) {}

    public function getName(): string
    {
        return 'RCT Bundle – rct_timeline fields to jsonData column';
    }
    ...
    public function shouldRun(): bool
    {
        ...
        $unmigrated = (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM tl_content
             ...
             AND (jsonData IS NULL OR JSON_EXTRACT(jsonData, '$.rct_timeline_color') IS NULL)",
            self::CE_TYPES
        );
        return $unmigrated > 0;
    }
    ...
    public function run(): MigrationResult
    {
        ...
        return $this->createResult(
            true,
            sprintf('rct_timeline → jsonData: %d Element(e) migriert.', $touched)
        );
    }
}
