...
class RctPeopleBoxJsonStorageMigration extends AbstractMigration
{
    private const FIELDS = [
        'rct_people_box_style',
        'rct_person_name',
        'rct_person_role',
        'rct_person_bio',
        'rct_person_email',
        'rct_person_phone',
        'rct_person_link',
        'rct_person_link_text',
    ];
    private const CE_TYPES = ['rct_people_box'];

    public function __construct(private readonly Connection $db) {}

    public function getName(): string
    {
        return 'RCT Bundle – rct_people_box fields to jsonData column';
    }
    ...
    public function shouldRun(): bool
    {
        ...
        $unmigrated = (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM tl_content
             ...
             AND (jsonData IS NULL OR JSON_EXTRACT(jsonData, '$.rct_person_name') IS NULL)",
            self::CE_TYPES
        );
        return $unmigrated > 0;
    }
    ...
    public function run(): MigrationResult
    {
        ...
        return $this->createResult(
            true,
            sprintf('rct_people_box → jsonData: %d Element(e) migriert.', $touched)
        );
    }
}
