...
class RctChartBarsJsonStorageMigration extends AbstractMigration
{
    private const FIELDS = [
        'rct_chart_bars_data',
        'rct_chart_orientation',
        'rct_chart_color',
        'rct_chart_show_values',
    ];
    private const CE_TYPES = ['rct_chart_bars'];

    public function __construct(private readonly Connection $db) {}

    public function getName(): string
    {
        return 'RCT Bundle – rct_chart_bars fields to jsonData column';
    }
    ...
    public function shouldRun(): bool
    {
        ...
        $unmigrated = (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM tl_content
             ...
             AND (jsonData IS NULL OR JSON_EXTRACT(jsonData, '$.rct_chart_orientation') IS NULL)",
            self::CE_TYPES
        );
        return $unmigrated > 0;
    }
    ...
    public function run(): MigrationResult
    {
        ...
        return $this->createResult(
            true,
            sprintf('rct_chart_bars → jsonData: %d Element(e) migriert.', $touched)
        );
    }
}
